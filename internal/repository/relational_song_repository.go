...
func (repository *RelationalSongRepository) FindByCode(ctx context.Context, code int64) (*model.Song, error) {
	...
	var err error
	var song model.Song
	if err = dao.ReadRowContext(ctx, repository.statementFindByCode, code, &song.Id, &song.Code, &song.Name, &song.ArtistId); err != nil {
		return nil, err
	}
...
func (repository *RelationalSongRepository) FindByName(ctx context.Context, name string) (*model.Song, error) {
	...
	var err error
	var song model.Song
	if err = dao.ReadRowContext(ctx, repository.statementFindByName, name, &song.Id, &song.Code, &song.Name, &song.ArtistId); err != nil {
		return nil, err
	}
